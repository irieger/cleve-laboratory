...
buttons = zeros(8,1);
bstrings = {'step','slow','fast','redo','next','random','quit','info'};
for k = 1:8
   buttons(k) = uicontrol('style','toggle','units','normal', ...
   'position',[.045+.115*(k-1) .005 .10 .045],'string',bstrings{k});
end
set(buttons(1),'userdata',0);
if pop.all, set(buttons([1:6 8]),'vis','off'), end
...
if ~isempty(bk)
   switch bk
      ...
      case 7  % quit
         repeat = false;
      case 8  % info
         web(['http://blogs.mathworks.com/cleve/2012/09/03/' ...
